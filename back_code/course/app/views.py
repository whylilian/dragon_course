...
def Create_match(request):
    ...
    # 比赛积分
    match_point = int(request.POST.get('match_point'))
    book_id = int(request.POST.get('book_id'))
    start_time = request.POST.get('start_time').strftime("%Y-%m-%d %H:%M:%S")
    end_time = request.POSt.get('end_time').strftime("%Y-%m-%d %H:%M:%S")
    match_time = request.POST.get('match_time')

    teacher = Teacher.objects.get(teacher_id = teacher_id)
    wordids = Classification.objects.filter(category_id = book_id).order_by('word_id').values('word_id')
